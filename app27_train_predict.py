...
from tensorflow.keras.models import Sequential, load_model  # load_model 임포트 추가
...
def create_ensemble_model(models, X):
    logging.info("모델들의 예측값을 통합합니다.")
...
def process_sheets_for_lstm(file_path, output_path, model_dir, output_graph_dir, metrics_output_path):
    ...
    all_metrics = []
    all_models = []  # 통합 모델에 사용할 개별 모델 저장
    ...
    for sheet_name in all_sheets:
        ...
        all_models.append(model)  # 개별 모델 저장
    ...
    # 앙상블 모델 예측을 위한 X 데이터 준비
    ensemble_X = np.concatenate([
        prepare_data_for_lstm(pd.read_excel(file_path, sheet_name=sheet_name), time_steps, future_days)[0] 
        for sheet_name in all_sheets
    ])

    # 개별 모델을 로드하고 예측 수행
    ensemble_predictions = []  # 앙상블을 위한 예측값 저장 리스트
    for sheet_name in all_sheets:
        model_path = os.path.join(model_dir, f'{sheet_name}_lstm_model.keras')
        model = load_model(model_path)  # 저장된 개별 모델 로드
        pred = model.predict(ensemble_X)  # 앙상블에 사용할 입력 데이터에 대해 예측 수행
        ensemble_predictions.append(pred)  # 예측값을 리스트에 저장

    # 각 모델의 예측값의 평균을 내어 최종 예측값을 생성
    final_predictions = np.mean(ensemble_predictions, axis=0)  # 평균을 통해 앙상블 예측값 생성

    # 통합 예측 결과를 데이터프레임으로 변환
    ensemble_df = pd.DataFrame({
        '예측 유입량(㎥/일)': final_predictions.flatten()  # 예측값을 데이터프레임 형태로 변환
    })

    # 통합 예측 결과를 Excel에 저장
    save_predictions_to_excel(ensemble_df, output_path, '통합_예측')

    # 앙상블 모델 저장
    ensemble_model_save_path = os.path.join(model_dir, 'ensemble_model.keras')
    # 앙상블 모델 생성 및 저장
    ensemble_model = Sequential()
    ensemble_model.add(Input(shape=(ensemble_X.shape[1], ensemble_X.shape[2])))  # 입력 레이어 정의
    ensemble_model.add(LSTM(50, return_sequences=True))  # 첫 번째 LSTM 레이어
    ensemble_model.add(Dropout(0.2))  # 드롭아웃 레이어 추가
    ensemble_model.add(LSTM(50))  # 두 번째 LSTM 레이어
    ensemble_model.add(Dropout(0.2))  # 드롭아웃 레이어 추가
    ensemble_model.add(Dense(1))  # 출력 레이어
    ensemble_model.compile(optimizer='adam', loss='mean_squared_error')  # 모델 컴파일

    # 앙상블 모델 훈련
    ensemble_model.fit(ensemble_X, final_predictions, epochs=100, batch_size=32, verbose=1)
    ensemble_model.save(ensemble_model_save_path)  # 앙상블 모델 저장
    logging.info(f"앙상블 모델을 저장했습니다: {ensemble_model_save_path}")

# 앙상블 학습 개요
"""
앙상블 학습(Ensemble Learning)은 여러 모델을 결합하여 예측 성능을 향상시키는 기법입니다.
- 서로 다른 오류를 범하는 모델들을 조합하여 더 안정적이고 정확한 예측을 목표로 합니다.
- 모델들이 다양한 데이터 샘플과 특성을 학습하여 다양성을 확보하는 것이 중요합니다.
- 예측값을 평균내어 최종 예측값을 생성함으로써 여러 모델의 강점을 결합합니다.
- 다양한 분야에서 활용되며, 예측의 정확성을 높이고 데이터의 노이즈에 강한 모델을 만드는 데 효과적입니다.
"""

if __name__ == '__main__':
    file_path = '취합_일자_강수량_인구_유입량_20240829_제공.xlsx'
    output_path = 'predicted.xlsx'
    model_dir = 'models'
    output_graph_dir = 'graphs'
    metrics_output_path = 'metrics.xlsx'

    # 데이터 전처리 실행
    process_data(file_path, 'Prepared.xlsx')

    # LSTM 모델 처리 실행
    process_sheets_for_lstm('Prepared.xlsx', output_path, model_dir, output_graph_dir, metrics_output_path)
